...
export const AttendanceComponent: React.FC<AttendanceComponentProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    const newEvent = {
      userId: Number(session?.user?.id), // Je zou hier de juiste gebruiker moeten toevoegen (bijvoorbeeld via een context of via login)
      title: title,
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          "Er is iets misgegaan bij het versturen van het verzoek."
        );
      }
      ...
      console.error(
        "Er is een fout opgetreden bij het toevoegen van attendance:",
        error
      );
